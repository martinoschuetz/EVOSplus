 ...
 *
 *   created:	Objective function reflection2, an optical problem
 *		refering to a privat correspondence with H.Greiner.
...
static	unsigned long	m = 201;	/* number of wavelengths */
static	double		XAir = 1.0,	/* refractory index of incident medium.
					   Default medium is air */
			XSub = 1.47,	/* refractory index of substrate.
					   Default medium is quartz. */
			lower  = 0.4,	/* Lower wavelength of the section */
			upper  = 2.0,   /* Upper wavelength of the section */
			border = 0.78,   /* border between two sections */
			stepsize = 8.0;
static double		RefInd[2] = {1.46, 2.2};/* Refractive indices */
static vector_tPtr	weights = NULL;
...
/*
 * reflectionInit2 -- convert parms to function local data.
 */

static struct option reflectionOpts2[] =
{
...
bool reflectionInit2(char *parms, population_tPtr pop)

{
	char		optarg[BUFSIZ];
	int		c;
	unsigned long	i;
	double		lambda, sam, x1, x5, planc;
	/* Some constants for calculating the intensity of a
	   'Planckscher Strahler' of temperatur T at wavelength lambda [nm].
	   This is used for calculating the weights. */ 
	   const double	c1 = 5.9547,	/* c1 = h * c**2 W (um)**2 */
			c2 = 1.4388e+7,	/* c2 = hc / k   K (nm)	   */
			T  = 2800.0;
	...
	if(first == FALSE) {
	  first = TRUE;
	  if(parms != NULL) {
	    while((c = getparm(parms, reflectionOpts2, optarg)) != EOF) {
		  switch (c) {
		  case 'l':
		  ...
		  case 'w':
			...
			break;
		  case 'b':
			border = (unsigned long) atoi(optarg);
			break;
		  case 'i':
	  ...
	  weights = veNewVector(m);
	  
	  for(i = 1; i <= m; i++) {
	    lambda = lower * 1000.0 + ((double)(i-1)) * stepsize;
		
	    if(lambda <= (border * 1000.0))
		  veAssignVectorComponent(i, 1.0, weights);
	    else {
		  x1	= lambda/100.0;
		  sam	= c2/(lambda * T);
		  sam	= 1.0/(exp(sam) - 1.0);
		  x5	= x1 * x1 * x1 * x1 * x1;
		  planc	= (10.0e+5 * 2.0 * c1 * sam) / x5;
		  /*		printf("%g %g\n", lambda, 3.0 * planc);
					veAssignVectorComponent(i, planc, weights);	*/
		  veAssignVectorComponent(i, 3.0 * planc, weights);
	    }
	  }
...
double reflection2(vector_tPtr x, Ivector_tPtr d)

{
    	unsigned long	i, j, k;
    	double 		ConsI, Trans, Wale, Delta,
			XMerit = 0.0,
	...
	for (i = 1; i <= m; i++) {
	    ...
	    Trans   = ConsI / (QQ1 * QQ1 + QQ2 * QQ2);

	    if(SpectralFlag)
		fprintf(spectralprofil, "%.10e\t\t%.10e\n", Wale/1000.0, Trans);

	    if(Wale <= (border * 1000.0))
	        XMerit = XMerit +
			 (veGetVectorComponent(i, weights) * stepsize *
			  (Trans - 1.0) * (Trans - 1.0));
	    else
	        XMerit = XMerit +
			 (veGetVectorComponent(i, weights) * stepsize *
...
bool reflectionTerm2(param_tPtr eps)

{
	...
	fprintf(spectralprofil, "\n\n# Merit = %g",
		reflection2(inGetX(eps->BstInd), inGetD(eps->BstInd)));
